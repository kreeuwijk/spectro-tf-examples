resource "spectrocloud_cluster_profile" "profile" {
  ...
  description = "Example profile for EKS cluster with EBS and EFS"
  tags        = ["iac:terraform"]
  cloud       = "eks"
  type        = "cluster"
  version     = "1.0.0"
    ...
    values       = templatefile("${path.module}/config/csi-aws-efs.yaml",
                    {
                      ...
                      efs-role-arn = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/iam-irsa-role-${var.name}"
                    })
  }
